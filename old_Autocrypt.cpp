...
/* Keeps on analysing all the functions untill no more variables are marked as sensitive.
   Calls the Transform function once analysis is finished*/
// @M --> Takes the module M as input and visit the Module for analysis


bool AutoCrypt::runOnModule(Module &M)
{
  bool fixed_point = false;   // Check if fixed point is reached
  int old_length, new_length; // Old and new length of encrypted variables size
...
//Change needed here for is a ConstantInt
...
//no need to worry about intrinsic calls
  ...
  if((*F).isDeclaration())
    {
    //isDeclaration True if it's not an inbuilt function
      if(!isVisited(fname))
	{
	  functionMap[fname] = InnerMap();
	  //nested map, change for adding type of the encrypted value
...
void AutoCrypt :: visitReturnInst(ReturnInst &I) 
{
...
void AutoCrypt :: visitPHINode(PHINode &I) 
{
...
void AutoCrypt :: visitGetElementPtr(GetElementPtrInst &I)
{     
...
void AutoCrypt :: visitLoadInst(LoadInst &I) 
{     
  LHSorRHS(&I);
...
void AutoCrypt ::  visitTruncInst(TruncInst &I) 
{    
  LHSorRHS(&I);
...
void AutoCrypt :: visitZExtInst(ZExtInst &I) 
{
  LHSorRHS(&I);
...
void AutoCrypt :: visitSExtInst(SExtInst &I) 
{
  LHSorRHS(&I);
}
...
//change for integers
void AutoCrypt :: visitStoreInst(StoreInst &I)
{     
  std::string fname = (I).getParent()->getParent()->getName();         // Name of the parent function
...
void AutoCrypt :: visitBinaryOperator(BinaryOperator &I)
{
  std::string fname = (I).getParent()->getParent()->getName();           // Name of the parent function
  std::string lhs = I.getName();                                         // Name of the L.H.S variable
  ...
  if(I.getOpcode() == Instruction::Add || I.getOpcode() == Instruction::FAdd || I.getOpcode() == Instruction::Sub || 
    ...
    // If FAdds and operands encrypted then throw error
...
//change here
void AutoCrypt :: visitICmpInst(ICmpInst &I) 
{
...
void AutoCrypt :: visitSwitchInst(SwitchInst &I) 
{
  std::string fname = (I).getParent()->getParent()->getName();           // Name of the parent function
...
// @enc_var_name --> Inserts this variable name to encrypted list
...
//Prepare for this in the analysis
//Need to encrypt the instructions
//Need to change the encryption mode from integer to bitwise
//add the call instrinsic
void AutoCrypt :: CallInsert(Instruction *I)
{
...
//What's this???
void AutoCrypt :: createPaillierFunc( Value *op1, Value *op2, StringRef name, Instruction *I)
{
  llvm::IRBuilder<> builder(getGlobalContext());
  Module *M = I->getParent()->getParent()->getParent();
  LLVMContext *ctx = &M->getContext(); 
